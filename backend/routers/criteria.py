...
def get_criteria_for_division(
    ...
    _# Bungkus peran dengan kurung siku [] agar terbaca sebagai 1 list (argumen tunggal)
    : object = Depends(require_role(["kepala_hrd", "kepala_cabang", "kepala_divisi"])),
):
    """
    Mengambil daftar kriteria aktif berserta bobot spesifiknya untuk sebuah sub-divisi.
    Mendukung dua format pemanggilan untuk fleksibilitas front-end:
    - /api/criteria?division_id=1
    - /api/criteria/division/1
    """
    if not division_id:
        raise HTTPException(
